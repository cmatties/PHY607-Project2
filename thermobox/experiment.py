...
def mb_pdf_speed(v, m, T):
    ...
    -------
    float
        Probability density at velocity v.
    """
    beta = m / (2 * kB * T)
...
def harmonic_equilibrium_checks(
    ...
    """Generate plots for harmonic potential particles
    ...
    m : float, optional
        Particle mass
    kx : float, optional
        X-direction spring constant
    ky : float, optional
        Y-direction spring constant
    Lx : float, optional
        Box width
    Ly : float, optional
        Box height
    T_bath : float, optional
        Ambient thermal bath temperature
    p_specular : float, optional
        Probabiilty of specular reflection
    seed : int, optional
    ...
    burn_in : int, optional
        Number of time steps to run before starting to record data
    """
...
def mb_speed_histogram(
    ...
    """Generate plots for speed distribution
...
def pressure_temperature(
    ...
    """Generate plots for pressure as a function of temperature
    ...
    box : float, optional
        Box side length
    T_list : float, optional
        List of bath temperatures to iterate over
    p_specular : float, optional
        Probabiilty of specular reflection
    dt : float, optional
        Time step size
    ...
    for T in T_list:
        ...
        r = rng.random(size=(N, 2)) * box
        v = gaussian_rejection_sampling(0, 0.5, rng, (N, 2))
