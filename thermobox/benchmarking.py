...
def benchmark(seed=1):
    """
    Utility function to produce time complexity plots
    ...
    for N in N_list:
        ...
        hard_particles = HardParticleList(r0, v0, Lx=box, Ly=box, seed=seed)
        harmonic_particles = HarmonicParticleList(r0, v0, Lx=box, Ly=box, seed=seed)
    ...
    plt.figure()
    plt.semilogy(
        N_list, np.array(harmonic_particle_times) / N_list, label="Harmonic Particles/N"
    )
    plt.semilogy(
        N_list, np.array(hard_particle_times) / N_list**2, label="Hard Particles/N^2"
    )
