...
def sample_velocity(T, m, rng):
    ...
    velocity = rng.normal(0, sigma, size=2) # 2D velocity
    return velocity


class Particle:
    def __init__(self, r, v, m=1.0, Lx=3.0, Ly=3.0, T=1.0, p_specular=0.6, seed=None):
    ...
    def wall_interact(self):
        ...
        if x < 0.0:
            ...
            if self.rng.random() < p:
                ...
                vx_new, vy_new = sample_velocity(T, m, self.rng)
                vx = abs(vx_new)
                vy = vy_new
            dp_sum += m * abs(vx - vx_before)
            x = 0.0
...
class HarmonicParticle(Particle):
    ...
    """
    def __init__(self, r, v, kx=1.0, ky=1.0, x0=1.5, y0=1.5, **kwargs):
...
class HardParticle(Particle):
    ...
    def particle_interact(self, other):
        ...
        self.v -= rel_speed_n * n
        other.v += rel_speed_n * n
